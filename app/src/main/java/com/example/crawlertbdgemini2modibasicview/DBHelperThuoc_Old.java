...
public class DBHelperThuoc_Old extends SQLiteOpenHelper {
...
//    private static final Lock lock3InitUrlsTable  = new ReentrantLock(); //Chỉ sử dụng 1 khóa lock cho tất cả phương thức ghi
    ...
    private static volatile DBHelperThuoc_Old instance;
...
//                    KY_TU_SEARCH + " TEXT, " +                //  Đưa vào table cha
    ...
    private DBHelperThuoc_Old(@Nullable Context context) {
    ...
    public static DBHelperThuoc_Old getInstance(Context ctx) {  // KHÔNG synchronized TÒAN HÀM SẼ NHANH HƠN
        if (instance == null) {
            synchronized (DBHelperThuoc_Old.class) {
                if (instance == null) {
                    instance = new DBHelperThuoc_Old(ctx.getApplicationContext());
                }
            }
        }
        return instance;
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE INDEX IF NOT EXISTS idx_thuoc_urlId ON " +
                TABLE_THUOC_2KT +"(" + DBHelperThuoc_Old.URL_ID +");"   //để tăng tốc join.
        );
        // Thêm index riêng cho maThuoc
        db.execSQL("CREATE INDEX IF NOT EXISTS idx_thuoc_maThuoc ON " +
                TABLE_THUOC_2KT +"(" + DBHelperThuoc_Old.MA_THUOC +");"
                );
...
//        CREATE INDEX IF NOT EXISTS idx_thuoc_url ON Thuoc(url);
        ...
        // Index để join nhanh
        db.execSQL("CREATE INDEX IF NOT EXISTS idx_thuoc_urlId ON " +
                TABLE_THUOC_3KT +"(" + DBHelperThuoc_Old.URL_ID +");"
        );
        // Thêm index riêng cho maThuoc
        db.execSQL("CREATE INDEX IF NOT EXISTS idx_thuoc_maThuoc ON " +
                TABLE_THUOC_3KT +"(" + DBHelperThuoc_Old.MA_THUOC +");"
        );
        ...
        // Index để join nhanh
        db.execSQL("CREATE INDEX IF NOT EXISTS idx_thuoc_urlId ON " +
                TABLE_THUOC_CUSTOM +"(" + DBHelperThuoc_Old.URL_ID +");"
        );
        // Thêm index riêng cho maThuoc
        db.execSQL("CREATE INDEX IF NOT EXISTS idx_thuoc_maThuoc ON " +
                TABLE_THUOC_CUSTOM +"(" + DBHelperThuoc_Old.MA_THUOC +");"
        );
    ...
    public void markUrlAsProcessing(String initUrlsTable, String url, int level) {   //Đang xử lý status = 1. Thêm tham số level
        // CŨ: KHÔNG DÙNG
        ContentValues values = new ContentValues();
        values.put(DBHelperThuoc_Old.LEVEL, level); // Của url cha thì level = 0 (Mặc định)
        values.put(DBHelperThuoc_Old.URL, url);
        values.put(DBHelperThuoc_Old.STATUS, 1); // Đang xử lý status = 1
        //Xem
...
//        cursor.close();
    ...
    private boolean isUrlExists(String initUrlsTable, String url) { // Cần không vì initUrlsTable có url là khóa chính
        ...
        try {
            ...
            cursor = db.query(initUrlsTable, new String[]{DBHelperThuoc_Old.URL}, DBHelperThuoc_Old.URL + " = ?", new String[]{url}, null, null, null);
    ...
    public void saveChildUrlToUrlQueueTable(String initUrlsTable,
        ...
        if (!isUrlExists(initUrlsTable, url)) { // Cần không vì initUrlsTable có url là khóa chính
            ...
            values.put(DBHelperThuoc_Old.LEVEL, level); //leve > 0 là url con
            values.put(DBHelperThuoc_Old.URL, url); // là khóa chính
            values.put(DBHelperThuoc_Old.STATUS, 0);    //Mặc định là 0 (chưa xử lý)
...
//            dbWriteLock.unlock();
    ...
    public boolean isMaThuocExists(String tableThuocCon, String maThuoc, String url) { // Cần không vì initUrlsTable có url là khóa chính
        ...
        if (tableThuocCon.endsWith("2kt")) {
            tableUrlCha = DBHelperThuoc_Old.TABLE_PARENT_URLS_2KT;
        } else if (tableThuocCon.endsWith("3kt")) {
            tableUrlCha = DBHelperThuoc_Old.TABLE_PARENT_URLS_3KT;
        } else {
            tableUrlCha = DBHelperThuoc_Old.TABLE_PARENT_URLS_CUSTOM;
        }
